...
func main() {
	...
	if len(cfg.Sorters) > 0 {
		...
		for _, sorterCfg := range cfg.Sorters {
			...
			// Configurar WebSocketHub y iniciar workers de estados de cajas
			...
			for i := range s.Salidas {
				...
				s.Salidas[i].StartBoxStatusWorker()
				log.Printf("        ✅ Worker de estados iniciado para Salida %d (physical_id=%d)",
